...
public class MainActivity extends Activity {
	...
	private void changeSlideMode(MenuItem item) {
		if (mSlideListView.getSlideMode() == SlideListView.SlideMode.BOTH) {
			mSlideListView.setSlideMode(SlideListView.SlideMode.LEFT);
		} else if (mSlideListView.getSlideMode() == SlideListView.SlideMode.LEFT) {
			mSlideListView.setSlideMode(SlideListView.SlideMode.RIGHT);
		} else if (mSlideListView.getSlideMode() == SlideListView.SlideMode.RIGHT) {
			mSlideListView.setSlideMode(SlideListView.SlideMode.NONE);
		} else if (mSlideListView.getSlideMode() == SlideListView.SlideMode.NONE) {
			mSlideListView.setSlideMode(SlideListView.SlideMode.BOTH);
		}
		if (item != null) {
			item.setTitle("切换SlideMode:" + mSlideListView.getSlideMode().toString());
		}
	...
	private void changeSlideLeftAction(MenuItem item) {
		if (mSlideListView.getSlideLeftAction() == SlideListView.SlideAction.SCROLL) {
			mSlideListView.setSlideLeftAction(SlideListView.SlideAction.REVEAL);
		} else {
			mSlideListView.setSlideLeftAction(SlideListView.SlideAction.SCROLL);
		}
		if (item != null) {
			item.setTitle("切换SlideLeftAction:" + mSlideListView.getSlideLeftAction().toString());
		}
	...
	private void changeSlideRightAction(MenuItem item) {
		if (mSlideListView.getSlideRightAction() == SlideListView.SlideAction.SCROLL) {
			mSlideListView.setSlideRightAction(SlideListView.SlideAction.REVEAL);
		} else {
			mSlideListView.setSlideRightAction(SlideListView.SlideAction.SCROLL);
		}
		if (item != null) {
			item.setTitle("切换SlideRightAction:" + mSlideListView.getSlideRightAction().toString());
		}
